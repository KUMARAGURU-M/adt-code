...
public class WorkwiseService {
        ...
        public List<MyTaskOption> getMyTaskOptions(UUID userId) {
                ...
                return tasks.stream().map(t -> {
                        ...
                        List<TaskJobAssignment> jobLinks = taskJobRepository.findByTaskId(t.getId());
        ...
        public WorkwiseContextResponse startTask(UUID userId,
                ...
                boolean alreadyRunning = timeLogRepository.existsByUserIdAndStatus(userId, "Running")
                ...
                if (isDevTask) {
                        ...
                        List<TaskJobAssignment> jobLinks = taskJobRepository.findByTaskId(task.getId());
        ...
        private boolean syncTaskStatusFromAssignments(Task task) {
                List<TaskEmployeeAssignment> all = taskEmployeeRepository.findByTaskId(task.getId());
        ...
        public WorkwiseContextResponse startBreak(UUID userId,
                ...
                if (request.getTimeLogId() == null) {
                        ...
                        // BUG FIX: If user has a running task (including devTask), reject — they must
                        // pass timeLogId.
                        boolean alreadyRunning = timeLogRepository.existsByUserIdAndStatus(userId, "Running")
                                        || timeLogRepository.existsByUserIdAndStatus(userId, "On Break");
                        if (alreadyRunning) {
        ...
        public Page<TimeLogResponse> getMyTimeLogs(UUID userId,
                ...
                return timeLogRepository.searchTimeLogs(
                                ...
                                status, startDate, endDate, pageable).map(this::toTimeLogResponse);
        }
        ...
        public List<TimeLogResponse> getAdminTimeLogs(UUID userId,
                ...
                return timeLogRepository.searchAdminTimeLogs(
                                userId, projectId, processId, status, startDate, endDate).stream()
                                .map(this::toTimeLogResponse).collect(Collectors.toList());
        }
                ...
                if (log.getTask() != null) {
                        ...
                        pages = task.getAssignedPagesStr() != null
                                        ...
                                        : (task.getAssignedPages() != null
                                                        ? task.getAssignedPages().toString()
                                                        : null);
        ...
        public CalendarStatsResponse getCalendarStats(UUID userId, int year, int month) {
                ...
                for (LocalDate date = startDate; !date.isAfter(endDate); date = date.plusDays(1)) {
                        ...
                        long workingSeconds = dayLogs.stream()
                                        .mapToLong(l -> l.getWorkingSeconds() != null ? l.getWorkingSeconds() : 0)
                                        .sum();
                        int pages = dayLogs.stream()
                                        .mapToInt(l -> l.getPagesCompleted() != null ? l.getPagesCompleted() : 0).sum();
                ...
                while (!current.isAfter(endDate)) {
                        ...
                        List<TimeLog> weekLogs = logs.stream()
                                        .filter(l -> !l.getLogDate().isBefore(finalStart)
                                                        && !l.getLogDate().isAfter(finalEnd))
                                        .collect(Collectors.toList());

                        long workingSeconds = weekLogs.stream()
                                        .mapToLong(l -> l.getWorkingSeconds() != null ? l.getWorkingSeconds() : 0)
                                        .sum();
                        int pages = weekLogs.stream()
                                        .mapToInt(l -> l.getPagesCompleted() != null ? l.getPagesCompleted() : 0).sum();
                ...
                for (java.util.Map.Entry<String, List<TimeLog>> entry : projectMap.entrySet()) {
                        long workingSeconds = entry.getValue().stream()
                                        .mapToLong(l -> l.getWorkingSeconds() != null ? l.getWorkingSeconds() : 0)
                                        .sum();
                        int pages = entry.getValue().stream()
                                        .mapToInt(l -> l.getPagesCompleted() != null ? l.getPagesCompleted() : 0).sum();
                ...
                // Monthly Summary
                long totalWorking = logs.stream()
                                .mapToLong(l -> l.getWorkingSeconds() != null ? l.getWorkingSeconds() : 0).sum();
                int totalPages = logs.stream().mapToInt(l -> l.getPagesCompleted() != null ? l.getPagesCompleted() : 0)
                                .sum();
        ...
        private void ensureAttendanceEmployeeExists(User user) {
                try {
                        AttendanceEmployee emp = attendanceEmployeeRepository.findByUserId(user.getId())
                                        .orElseGet(() -> {
                                                String fullName = user.getEmployeeProfile() != null
                                                                ...
                                                                : null;
                                                if (fullName != null) {
                                                        List<AttendanceEmployee> employees = attendanceEmployeeRepository
                                                                        .searchEmployees(null, fullName.trim());
                        ...
                        if (emp == null) {
                                ...
                                log.info("Auto-created AttendanceEmployee '{}' for user during Workwise task start",
                                                emp.getName());
                        }
                } catch (Exception e) {
        ...
        private TimeLogResponse toTimeLogResponse(TimeLog log) {
                ...
                if ("Running".equals(log.getStatus()) || "On Break".equals(log.getStatus())) {
                        ...
                        java.util.Optional<BreakLog> activeBreakOpt = log.getBreakLogs() == null
                        ...
                        // IMPORTANT: preserve the actual status ("Running" or "On Break") — do NOT
                        // override it.
                ...
                List<TimeLogResponse.BreakLogDto> breakLogs = log.getBreakLogs() == null
                                ...
                                : log.getBreakLogs().stream()
                                                .map(b -> TimeLogResponse.BreakLogDto.builder()
                                                                ...
                                                                .durationSeconds(b.getDurationSeconds() != null
                                                                                ? b.getDurationSeconds()
                                                                                : (b.getBreakEnd() != null
                                                                                                ? (int) java.time.temporal.ChronoUnit.SECONDS
                                                                                                                .between(b.getBreakStart(),
                                                                                                                                b.getBreakEnd())
                                                                                                : (int) java.time.temporal.ChronoUnit.SECONDS
                                                                                                                .between(b.getBreakStart(),
                                                                                                                                OffsetDateTime.now())))
                                                ...
                                                .sorted(java.util.Comparator
                                                                .comparing(TimeLogResponse.BreakLogDto::getBreakStart))
                                                .collect(Collectors.toList());
                ...
                try {
                        attendanceEmployeeRepository.findByUserId(log.getUser().getId()).ifPresent(emp -> {
                                attendanceRecordRepository
                                                .findByEmployeeIdAndAttendanceDate(emp.getId(), log.getLogDate())
                                                .ifPresent(rec -> {
                        ...
                        org.slf4j.LoggerFactory.getLogger(WorkwiseService.class)
                                        .warn("Could not fetch manual check-in/out: {}", e.getMessage());
                }
                ...
                return TimeLogResponse.builder()
                                ...
                                .totalPages(
                                                log.getJob() != null && log.getJob().getPageCount() != null
                                                                ? log.getJob().getPageCount()
                                                                : log.getTask() != null && log.getTask()
                                                                                .getTotalPages() != null ? log.getTask()
                                                                                                .getTotalPages()
                                                                                                : null)
                                .status(log.getStatus())
